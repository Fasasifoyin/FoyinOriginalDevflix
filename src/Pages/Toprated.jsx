...
const Toprated = () => {
    const {data2, loading, error, newData2, bottomPage} = useFetchData('movie/top_rated')
    const aa = [...newData2, ...data2]

    useEffect(() => {
      document.title = 'Discover - Toprated Movies'
    }, [])

  return (
    <div className='px-3 px-lg-4 py-4 mt-5' >
    <h3 className='mb-4'>Top Rated Movies</h3>
    <Row className='gy-2'>
            {aa.map((each, index) => (
              aa.length === index + 1 ? (
